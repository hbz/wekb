  ...
  def work(Job aJob) {
    ...
    try {
      ...
      if(tippDuplicates.size() > 0){
        ...
        tippDuplicates.each {
          if(!(it in tippsFound)){
            KBComponent.executeUpdate("update KBComponent set status = :deleted, lastUpdated = ${new Date()} where id = (:tippId) and status != :deleted", [deleted: status_deleted, tippId: it])
          }
        }
      ...
      if (!cancelled) {
        ...
        if(setAllTippsNotInKbartToDeleted){
          ...
          Integer tippsToDeleted = tippsIds ? KBComponent.executeUpdate("update KBComponent set status = :deleted, lastUpdated = ${new Date()} where id in (:tippIds) and status != :deleted", [deleted: status_deleted, tippIds: tippsIds]) : 0
